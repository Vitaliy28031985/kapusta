...
const GraphsMobile = ({data}: GraphsProps) => {
  ...
  return (
      <div className="relative z-50  p-6 bg-white rounded-lg">
    
      {data.map((item, i) => {
        const minWidth = (item.value / maxValue) * 200 >= 80;
       return (
        <div key={item.label} className="relative z-50 mb-5 ">
           {minWidth ? (
             <div className="mb-2 flex items-center justify-between"
            style={{
              width: `${(item.value / maxValue) * 200 + 15}px`,
            }}>
            <p className="text-xs text-text_color">{item.label}</p>
            <p className="text-xs text-text_color">{`${item.value} UAH`}</p>
             </div>) : (
                <div className="mb-2 flex items-center justify-between"
            style={{
              width: `${(item.value / maxValue) * 200 + 70}px`,
            }}>
            <p className="text-xs text-text_color">{item.label}</p>
            <p className="text-xs text-text_color">{`${item.value} UAH`}</p>
          </div>
          ...
          <div
            className="h-4 rounded-br-[10px] rounded-tr-[10px] "
            style={{
